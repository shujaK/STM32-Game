...
void draw_bullet(frame *f, bullet *b)
{
  ...
  const uint8_t* s;
  if (b->damage == 1) s = &enemy_bullet_sprite;
  if (b->damage == 2) s = &enemy_bullet_sprite_2;
  draw_sprite(f, b->p.x, b->p.y, s, ENEMY_BULLET_SPRITE_WIDTH_PACKED, ENEMY_BULLET_SPRITE_WIDTH, ENEMY_BULLET_SPRITE_HEIGHT);
}
